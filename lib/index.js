...
// dsh-vision-router 的 wrapper 路由 deepseek-vision 底层包装 deepseek-official，
// 同样消耗 DeepSeek 官方余额，一并归入。
function isDeepSeekProvider(p) {
  return p === PROVIDER_DEEPSEEK || p === "deepseek-official" || p === "deepseek-vision";
}
...
function apply(ctx) {
  ...
  // 当前选中 provider/model。优先级（对齐 host-apiproxy 的 selectionFor）：
  // 1) session 最近一次请求的 provider/model（agent.session.requestHeader().config）
  // 2) 全局默认（agentDefaultModel.currentSelection()）
  // 否则切了会话却没改全局默认时，会错读成全局默认的 provider。
  function currentSelection(sessionId) {
    if (sessionId) {
  ...
  async function deepseekStatus(sessionId, model, provider) {
  ...
  async function kimiStatus(model) {
  ...
  async function opencodeStatus(model) {
  ...
  async function zhipuStatus(provider, model) {
  ...
  async function minimaxStatus(provider, model) {
  ...
  async function openrouterStatus(model) {
  ...
  async function codexStatus(model) {
  ...
  async function getStatus(sessionId) {
    const sel = currentSelection(sessionId);
    const provider = sel ? sel.provider : null;
    const model = sel ? sel.model : null;

    if (isDeepSeekProvider(provider)) return deepseekStatus(sessionId, model, provider);
    if (provider === PROVIDER_KIMI) return kimiStatus(model);
    if (provider === PROVIDER_OPENCODE) return opencodeStatus(model);
    if (provider === "zai-coding-cn" || provider === "zai") return zhipuStatus(provider, model);
    if (provider === "minimax-cn" || provider === "minimax") return minimaxStatus(provider, model);
    if (provider === "openrouter") return openrouterStatus(model);
    if (provider === "openai-codex") return codexStatus(model);
...
export { apply, inject, toNum, round2, isDeepSeekModel, isDeepSeekProvider, codexWindowName };
